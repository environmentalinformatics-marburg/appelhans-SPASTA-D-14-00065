source("global_prep_annual_ndvi.R")
...
save(model, file = paste(mthd, "_interp_train_model_annual_ndvi.rda", sep = ""))

### interpolation data for prediction
for (i in seq(month.indx)) {
  cat("interpolating", pred.vars.unique$year[i], pred.vars.unique$month[i],
      "\n", sep = " ")
  ...
  nm <- paste("ki_", resp.var, "_", mthd, "_interp_", 
              "annual_ndvi_", pred.vars.unique$datetime[i],
