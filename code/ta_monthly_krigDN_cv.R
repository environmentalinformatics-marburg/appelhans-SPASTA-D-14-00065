...
for (j in seq(ta.pred.splt.folds)) {
  ...
  for (i in seq(ta.pred.splt.folds[[j]]$train)) {
    ...
    cat("\n========================================================\n",
        "\n", "krigDN year ", unique(test@data$year),
        " month ", unique(test@data$month), ": run ", j, "-", i, " of ",
        length(ta.pred.splt.folds), "(10)", "\n",
        "\n========================================================", "\n\n",
        sep = "")

    fmla <- as.formula(paste("resp ~ dem + ndvi_tr"))
    ...
    dir.create(paste("results", "krigDN", sep = "/"), showWarnings = FALSE)
    ...
    nm_out <- paste("results/krigDN/ki_krigDN_ta_dem_ndvi",
