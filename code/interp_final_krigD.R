...
for (j in seq(ta.pred.splt)) {
  ...
  cat("\n========================================================\n",
      "\n", "krigD year ", unique(train@data$year),
  ...
  nm <- paste("ki_", "Ta_200", "_", "krigD", "_interp_",
              unique(ta.pred.splt[[j]]$datetime),
              "_utm37s_wgs84", ".tif", sep = "")

  out.dir <- paste("interp_maps", "krigD", sep = "/")
  ...
  save(krig_res, file = paste("interp_maps/krigD/", mthd,
                              "_interp_final_krigD_model_",
