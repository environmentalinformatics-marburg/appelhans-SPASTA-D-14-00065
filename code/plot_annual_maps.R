...
ann_mean_plts <- lapply(seq(ann_mean), function(i) {
  ...
  labs <- c("gbm", "cubist", "rf", "avNNet", "krigD", "krigDSN")

  p <- spplot(ann_mean[[i]], col.regions = envinmrPalette(1000),
              at = seq(-8, 33, 0.1),
...
out <- latticeCombineGrid(ann_mean_plts, layout = c(2, 3))

png("graphs/maps.png", width = 25, height = 30, units = "cm", res = 300)
plot.new()
print(out)
dev.off()
